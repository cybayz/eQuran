...
class Model {
    ...
    protected function delete($table, array $fields) {
    ...
    protected function getfromdb($table, array $where = []) {
        $data = $this->Db->select($table, $where);
        if ($data->count()) {
    ...
    protected function getfromdbusingselect($query, array $parrams = []) {
        $data = $this->Db->query($query, $parrams);
        if ($data->count()) {
            $this->data = $data->results();
        }
        return $this;
